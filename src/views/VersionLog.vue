<template>
    <div style="display: flex; flex-direction: row;">
        <div style="display: flex; flex-direction: column; flex:flex-shrink; width: 200px">
            <div style="flex:auto; cursor:pointer; border-width: thin; border-style: solid; height: 120px" v-for="update in updateLog" :key="update.version"
                @click="selectUpdate(update.version)">
                <h2>{{ update.version }}</h2>
                <p>{{ update.date }}</p>
            </div>
        </div>
        <div style="flex:flex-grow;" id="displayUpdateDiv">
